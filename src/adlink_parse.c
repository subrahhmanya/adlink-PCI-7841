...
/**
 * read input parser and write output formatter
 */
...
int
pcan_make_output (char *buffer, TPCANRdMsg * m)
{
    u32 rest = 8;
    char *ptr = buffer;
    int i;
...
static int
skip_blanks_and_test_for_CR (char **ptr)
{
    /* remove blanks or tabs */
    skip_blanks (ptr);
...
static int
scan_unsigned_number (char **ptr, u32 * dwResult)
{
    char *p = *ptr;
...
int
pcan_parse_input_idle (char *buffer)
{
    char *ptr = buffer;
...
int
pcan_parse_input_message (char *buffer, TPCANMsg * Message)
{
    char *ptr = buffer;
    u32 dwLen;
    u32 dwDat;
...
int
pcan_parse_input_init (char *buffer, TPCANInit * Init)
{
    char *ptr = buffer;
    int i = 0;
    u32 dwDummy;
