...
static int
copy_from_user_rt (rtdm_user_info_t * user_info, void *to, void *from, size_t size)
{
    if (user_info)
    {
...
static int
copy_to_user_rt (rtdm_user_info_t * user_info, void *to, void *from, size_t size)
{
    if (user_info)
    {
        if (!rtdm_rw_user_ok (user_info, to, size) || rtdm_copy_to_user (user_info, to, from, size))
            return -EFAULT;
    }
    else
        memcpy (to, from, size);
...
static void
wait_until_fifo_empty (struct pcandev *dev, u32 mTime, struct pcanctx_rt *ctx)
{
    int fifo_not_empty;
...
int
pcan_open_rt (struct rtdm_dev_context *context, rtdm_user_info_t * user_info, int oflags)
{
    struct pcandev *dev;
    int err = 0;
...
int
pcan_close_rt (struct rtdm_dev_context *context, rtdm_user_info_t * user_info)
{
    struct pcandev *dev;
    struct pcanctx_rt *ctx;
...
int
pcan_ioctl_read_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPCANRdMsg * usr)
{
    int err = 0;
    TPCANRdMsg msg;
...
int
pcan_ioctl_write_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPCANMsg * usr)
{
    int err = 0;
    TPCANMsg msg;
    struct pcandev *dev;
...
int
pcan_ioctl_extended_status_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx,
...
int
pcan_ioctl_status_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPSTATUS * status)
{
    int err = 0;
    struct pcandev *dev;
    TPSTATUS local;
...
int
pcan_ioctl_diag_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPDIAG * diag)
{
    int err = 0;
    struct pcandev *dev;
    TPDIAG local;
...
int
pcan_ioctl_init_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPCANInit * Init)
{
    int err = 0;
    TPCANInit local;
    struct pcandev *dev;
...
int
pcan_ioctl_BTR0BTR1_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx, TPBTR0BTR1 * BTR0BTR1)
{
    int err = 0;
    TPBTR0BTR1 local;
...
int
pcan_ioctl_msg_filter_rt (rtdm_user_info_t * user_info, struct pcanctx_rt *ctx,
                          TPMSGFILTER * filter)
{
    ...
    return pcan_add_filter (dev->filter, local_filter.FromID, local_filter.ToID,
                            local_filter.MSGTYPE);
}

/* is called at user ioctl() call */
int
pcan_ioctl_rt (struct rtdm_dev_context *context, rtdm_user_info_t * user_info,
               IOCTL_REQUEST_TYPE request, void *arg)
{
    int err;
    struct pcanctx_rt *ctx;
    struct pcandev *dev;
